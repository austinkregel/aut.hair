...
const createApiToken = () => {
    createApiTokenForm.post(route('passport.personal.tokens.store'), {
        preserveScroll: true,
        onSuccess: () => {
...
const manageApiTokenPermissions = (token) => {
    console.log(token);
    updateApiTokenForm.scopes = token.scopes;
...
const deleteApiToken = () => {
    deleteApiTokenForm.delete(route('passport.personal.tokens.destroy', apiTokenBeingDeleted.value), {
        preserveScroll: true,
        preserveState: true,
    ...
    <div>
                    ...
                    <div class="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div v-for="permission in availablePermissions" :key="permission.id">
                            <label class="flex items-center">
                                <JetCheckbox v-model:checked="createApiTokenForm.scopes" :value="permission" />
        ...
        <JetDialogModal :show="displayingToken" @close="displayingToken = false">
            ...
            <template #content>
                ...
                <div v-if="$page.props.jetstream.flash.token" class="mt-4 bg-slate-100 px-4 py-2 rounded font-mono text-sm text-slate-500 dark:text-slate-400">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div v-for="permission in availablePermissions" :key="permission">
              <label class="flex items-center">
                ...
                <span class="ml-2 text-sm text-slate-600 dark:text-slate-200">{{ permission }}</span>
              </label>
            </div>
          </div>
        </template>
